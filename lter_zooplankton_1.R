...
nes_zoo_wide <- nes[,c("cruise_name","date","station","zoo_gear","ich_gear","lat","lon","time","depth","sfc_temp","sfc_salt","btm_temp","btm_salt","volume_1m2","calfin_10m2","ctyp_10m2","pseudo_10m2","tlong_10m2","larvaceans_10m2","para_10m2","hyper_10m2")]
#make a vector with desired columns 
#select only the seven taxa of interest (calanus finmarchicus, centropages typicus, 
#pseudocalanus, paracalanus, temora longicornis, larvacians, and hyperiids, could not 
#find metridia)

library(tidyr)
nes_zoo_long <- gather(nes_zoo_wide, taxa, organisms_per_10m2, calfin_10m2:hyper_10m2, factor_key=TRUE)
