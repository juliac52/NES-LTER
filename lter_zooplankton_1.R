...
EcoMon_ymd <- EcoMon %>% separate(date, sep ="-", into = c("year", "month", "day"))
#separate date into year, month, and day 
install.packages("data.table")
library(data.table)
nes_lat <- setDT(EcoMon_ymd)[!(lat %between% c(0, 39.5) | lat %between% c(41.4, 50))]
...
nes_zoo_wide <- nes[,c("cruise_name","year","month","day","station","zoo_gear","ich_gear","lat","lon","time","depth","sfc_temp","sfc_salt","btm_temp","btm_salt","volume_1m2","calfin_10m2","ctyp_10m2","pseudo_10m2","tlong_10m2","larvaceans_10m2","para_10m2")]
...
#attempt to see what dates are missing
